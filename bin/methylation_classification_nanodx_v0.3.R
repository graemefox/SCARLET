...
fileout <- paste0(opt$sample,"_methylation_hg38_HM450.RDS")
...
write.csv(probes,file = paste0(opt$sample,"_probes_for_training.csv"),row.names = FALSE, col.names = FALSE,quote = FALSE)
...
details <- paste(paste0("450k_overlap: ", length(unique(names(case)))), 
                           ...
                           sep = "\n")
write.table(details, file = paste0(opt$sample,"_rf_details.tsv"), row.names = F, col.names = F, quote = F)
...
report <- paste(paste0("Number of features: ", rf$num.independent.variables), 
                ...
                sep = "\n")
write.table(report, file = paste0(opt$sample,"_calibrated_classification.tsv"), row.names = F, col.names = F, quote = F)

write.table(votes, file = paste0(opt$sample,"_votes.tsv"), quote = F)
