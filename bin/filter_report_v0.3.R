...
# it is possible for everything to have been filtered out by this point

var_file_size <- file.info(opt$input)$size
...
# else, the file was empty, so write an empty output file also
} else {
    headers <- c("Chr","Start","End","Func","Gene","ExonicFunc","AAChange","cytoBand","1000g_EUR","COSMIC")
    write.csv(headers, file=opt$output,row.names=F)
}
